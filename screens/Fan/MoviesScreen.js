...
const MoviesScreen = () => {
  ...
  const isFav = selectedMovie && favorites.find(f => f.id === selectedMovie.id);
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#1b1b1b' : '#e8f0e6' }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const fav = favorites.find(m => m.id === item.id);
          return (
        ...
        <ScrollView style={[styles.modalContainer, { backgroundColor: isDark ? '#1e1e1e' : '#fff' }]}>
          {selectedMovie && (
            <>
              ...
              <Text style={[styles.text, { color: isDark ? '#ccc' : '#333' }]}>
                🎥 Productor: {selectedMovie.producer}
              </Text>
              <Text style={[styles.text, { color: isDark ? '#ccc' : '#333', marginTop: 10 }]}>
